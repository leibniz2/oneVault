...
package beta.com.android17.onevault.FileChooser;
...
class FileChooserCore {
	...
	/**
	 * Implementation of the click listener for when the icon_add button is clicked.
	 */
	 ...
	 * 
	 * @param listener The listener to icon_add.
	 */
	...
	public void setLabels(FileChooserLabels labels) {
		...
		// Verify if the buttons for icon_add a file or select a folder has been modified.
		if(labels != null) {
			LinearLayout root = this.chooser.getRootLayout();
	...
	public void loadFolder(File folder) {
		...
		if(this.currentFolder.exists() && layout != null) {
			...
			if(this.currentFolder.isDirectory()) {
				...
				if(fileList != null) {
					...
					for(int i=0; i<fileList.length; i++) {
						...
						if(selectable || !this.showOnlySelectable) {
							// Create the file item and icon_add it to the list.
				...
				// The file is not a folder, icon_add only this file.
				fileItems.add(new FileItem(this.chooser.getContext(), this.currentFolder));
			}
	
			
			// Add click listener and icon_add the FileItem objects to the layout.
